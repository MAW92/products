function parseJSON(jsonData) {
  ...
  parsedData[features,rarity,cardCondition].ItemSpecifics.NameValueList.forEach((item) => {
...
function filterByFeatures(features) {
  iso.arrange({
    filter: function (item) {
      const itemFeatures=item.querySelector('.features').textContent;
      return features.includes(itemFeatures);
    }
  });
}
...
function filterByRarity(rarity) {
  iso.arrange({
    filter: function (item) {
      const itemRarity=item.querySelector('.rarity').textContent;
      return rarity.includes(itemRarity);
    }
  });
}

// Filter by Card Condition
function filterByCardCondition(cardCondition) {
  iso.arrange({
    filter: function (item) {
      const itemCardCondition=item.querySelector('.card-condition').textContent;
      return cardCondition.includes(itemCardCondition);
    }
  });
}
