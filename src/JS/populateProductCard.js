    ...
    .then(data => {
      ...
      for(let i=0;i<products.length;i++) {
        ...
        productCard.innerHTML=`
          <a href="${listingData.viewItemURL}"
            target="_blank"
            title="View on eBay"
            ...
            <div class="w-fit border border-zinc-50 backdrop rounded-lg shadow-lg overflow-ellipsis will-change-transform hover:transform-gpu hover:duration-500 hover:ease-in-out hover:scale-105 hover:bg-gradient-to-b hover:from-transparent hover:to-transparent hover:via-black hover:text-shadow text-white">
              <img src="${listingData.galleryURL}"
                alt="${listingData.title}"
                class="w-full h-72 object-cover object-top rounded-t-lg">
              <div class="p-2 flex-wrap">
                <h3 class="text-lg font-black text-shadow text-white">${listingData.title}</h3>
                <p class="mt-2 text-shadow font-bold text-white">${cardData.Price}</p>
                <p class="text-shadow text-sm font-bold text-white">Shipping: $${cardData.Shipping}</p>
                <p class="mt-5 text-sm italic font-semibold text-shadow text-white">${cardData.Condition} condition, kept sleeved and stored in a safe environment.</p>
              </div>
            </div>
          </a>`;
